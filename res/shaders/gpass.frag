...
layout(location = 6) uniform samplerCube cubeAmbient;
...
void main()
{
	//vec2 gridSpacing = vec2(1, 1) * 0.02f;
	//float tol = 0.005f;
	//vec2 snap = mod(vUV, gridSpacing);

	////snap.x < tol || snap.y < tol || snap.z < tol
	//if(snap.x > tol || snap.y > tol)
	//	discard;
	...
	normal   = vec4(N,0);
	specular = texture(cubeAmbient, N);//= texture(specularMap, vUV);
